...
char *ZERO=new char[2], *ONE=new char[2];
char *NINE=new char[2], *TEN=new char[10];
char *TWO=new char[2];
...
tempresults[i].num[j]=tempresults[i].num[j+1];
       ...
       char * operator - (bigint other){ //returns only the difference b/w two no.s
           ...
           for(j=0;j<k-i+1;j++)
               result[j]=result[j+i];
           if(strlen(result)==0){ char *s=new char[2]; s[0]='0'; s[1]='\0'; return s;};
       ...
       char * operator ^ (bigint other){
           bigint result(ONE),one(ONE), zero(ZERO);
           int i;
           bigint l(1);
           bigint thisone(len);
           char *resultstring;
       ...
       char * operator /( bigint dividend){
            bigint x,zero(ZERO),one(ONE),nine(NINE),ten(ZERO), divisor,current,s,remainder;
       ...
       void sqrt(bigint x){
             ...
             bigint rem(ONE),temp,k,ss,zero(ZERO),
divisor=zero,mod,sq,nine(NINE),ten(TEN);
             if(x.len%2==1)
                    take=1;
       ...
       char * operator %( bigint other){
               bigint zero(ZERO),div(len),mod,x,result;
               int i;
               char *resultstring;
