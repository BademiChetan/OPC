...
struct Solver
{
	...
	SegmentTree tree;
	void recurse(int idx, vector < pair <int, LL> >& wv, vector <int>& f, vector < pair <int, LL> >& result, vector <int>& content, LL w, LL v) {
		if (idx == size(wv)) {
			if (w <= 1e8)
			...
			for (content[idx] = 0; content[idx] <= f[idx]; ++content[idx])
				recurse(idx + 1, wv, f, result, content, w + wv[idx].first * content[idx], v + wv[idx].second * content[idx]);
		}
	}
	void recurseX(int W, LL& res, int idx, vector < pair <int, LL> >& wv, vector <int>& f, vector < pair <int, LL> >& result, vector <int>& content, LL w, LL v) {
		if (idx == size(wv)) {
			if (w <= 1e8) {
				LL w_i = w, v_i = v;
				if (W - w_i >= 0) {
			...
			for (content[idx] = 0; content[idx] <= f[idx]; ++content[idx])
				recurseX(W, res, idx + 1, wv, f, result, content, w + wv[idx].first * content[idx], v + wv[idx].second * content[idx]);
		}
	}
	void build(vector < pair <int, LL> >& wv, vector <int>& f, vector < pair <int, LL> >& result) {
		vector <int> content(size(f), 0);
		recurse(0, wv, f, result, content, 0, 0);
	}
	void buildX(int W, LL& res, vector < pair <int, LL> >& wv, vector <int>& f, vector < pair <int, LL> >& result) {
		vector <int> content(size(f), 0);
		recurseX(W, res, 0, wv, f, result, content, 0, 0);
	}
	...
	LL solve(LL W, vector < pair <int, LL> >& left_wv, vector < pair <int, LL> >& right_wv,
			...
			LL result = 0;
			buildX(W, result, left_wv, left_f, wvL);
			return result;
	}
