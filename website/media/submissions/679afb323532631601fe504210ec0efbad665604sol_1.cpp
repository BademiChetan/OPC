...
struct SegmentTree
{
	...
	void initialize(vector <LL>& v) {
		...
		m.resize(N, INF); recurse(0, size(v) - 1, 1);
	}
...
struct Solver
{
	void recurse(int idx, vector < pair <int, int> >& wv, vector <int>& f, vector < pair <LL, LL> >& result, vector <int>& content) {
		if (idx == size(wv)) {
			LL w = 0, v = 0;
			for (int i = 0; i < idx; ++i)
	...
	void build(vector < pair <int, int> >& wv, vector <int>& f, vector < pair <LL, LL> >& result) {
		vector <int> content(size(f), 0);
		recurse(0, wv, f, result, content);
	}
	int binarySearch(vector < pair <LL, LL> >& wv, int low, int high, LL value) {
		// mid <= value && mid + 1 < value
		if (wv[high].first <= value) return high;
		while (low <= high) {
			int mid = (low + high) >> 1;
			if (wv[mid].first <= value && wv[mid + 1].second > value) return mid;
			else if (wv[mid].first <= value) low = mid + 1;
			else high = mid - 1;
		}
		return -1;
	}
	LL solve(LL W, vector < pair <int, int> >& left_wv, vector < pair <int, int> >& right_wv,
				vector <int>& left_f, vector <int>& right_f) {
			///////////////////////////////////////////////////
			vector < pair <LL, LL> > wvL, wvR;
	...
	LL solve(int N, int W, vector < pair <int, pair <int, int> > >& input) {
		...
		vector < pair <int, int> > left_wv, right_wv;
