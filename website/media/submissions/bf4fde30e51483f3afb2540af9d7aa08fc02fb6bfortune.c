...
int main()
{
	llu t,n,i,j=0,k,power,count=0,rem,tot;
	llu remainder1[40];
	llu remainder2[40];
	scanf("%llu",&t);
	for(i=2;i<=limit;i++)
	...
	while(t--)
	{
		bzero(remainder1,sizeof(remainder1));
		bzero(remainder2,sizeof(remainder2));
		scanf("%llu %llu",&n,&k);
		tot=phi(k);
		fill(remainder1,tot);
		fill(remainder2,k);
		//2^n mod phi(k)
		rem=call(remainder1,n,tot);
		//2^ mod rem
		power=call(remainder2,rem,k);
		printf("%llu\n",power);
	}
	return 0;
}
