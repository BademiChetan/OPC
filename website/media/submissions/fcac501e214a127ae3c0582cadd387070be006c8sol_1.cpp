...
struct Solver
{
	int solve(int x, int y, int N = 20, int result = 0) {
		if (N == 0) return result;
		else {
			int newN = N - 1;
			if (x < (1 << newN) && y < (1 << newN))
				return solve(x , y, newN, result);
			else if (x >= (1 << newN) && y >= (1 << newN))
				return solve(x - (1 << newN), y - (1 << newN), newN, result);
			else if (x >= (1 << newN) && y < (1 << newN))
				return solve(x - (1 << newN), y, newN, result + (1 << newN));
			else
				return solve(x, y - (1 << newN), newN, result + (1 << newN));
		}
	}
};
