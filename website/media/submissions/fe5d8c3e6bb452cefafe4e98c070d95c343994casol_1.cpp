...
struct Solver
{
	...
	void recurseX(int W, LL& res, int idx, vector < pair <int, LL> >& wv, vector <int>& f, vector < pair <int, LL> >& result, vector <int>& content) {
		if (idx == size(wv)) {
			LL w = 0, v = 0;
			for (int i = 0; i < idx; ++i)
				w += wv[i].first * content[i], v += wv[i].second * content[i];
			...
			for (content[idx] = 0; content[idx] <= f[idx]; ++content[idx])
				recurseX(W, res, idx + 1, wv, f, result, content);
		}
	}
	...
	void buildX(int W, LL& res, vector < pair <int, LL> >& wv, vector <int>& f, vector < pair <int, LL> >& result) {
		vector <int> content(size(f), 0);
		recurseX(W, res, 0, wv, f, result, content);
	}
